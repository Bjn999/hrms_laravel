...
class Main_salary_employee_absenceController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("admin.main_salary_employee_absences.index", ["data" => $data, 'finance_years' => $finance_years]);
    }
    ...
    public function show($id)
    {
        ...
        if (empty($finance_month_data)) {
            return redirect()->route('mainsalaryabsence.index')->with(['error' => 'عفواً غير قادر على الوصول الى البيانات المطلوبة']);
        }

        // Bring all absences for the specific Finance Month.
        $absences_data = get_cols_where(new Main_salary_employee_absence(), array("*"), array("com_code" => $com_code, "finance_months_periods_id" => $id), "id", "DESC");
        ...
        return view('admin.main_salary_employee_absences.show', ['data' => $absences_data, 'financeMonth_data' => $finance_month_data, 'employees' => $employees, 'employees_for_search' => $employees_for_search]);
    }

    // Check If the employee has absences before for this finance month -> Ajax
    public function checkExist(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->id;
            $checkExist = get_count_where(new Main_salary_employee_absence(), array('com_code' => $com_code, 'employee_code' => $request->employee_code, 'finance_months_periods_id' => $request->finance_month_period_id));
            if ($checkExist > 0) {
    ...
    public function edit(Request $request)
    {
        try {
            if ($request->ajax()) {
                ...
                $mainSalaryabsence_data = get_cols_where_row(new Main_salary_employee_absence(), array('*'), array('com_code' => $com_code, 'id' => $request->id, 'finance_months_periods_id' => $request->finance_month_period_id, 'main_salary_employee_id' => $request->main_salary_employee_id, 'is_archived' => 0));
                if (!empty($financeMonth_data) and !empty($mainSalaryEmployee_data) and !empty($mainSalaryabsence_data)) {
                    ...
                    return view('admin.main_salary_employee_absences.edit_absence', ['absence_data' => $mainSalaryabsence_data, 'employees' => $employees]);
                }
            }
        } catch (\Exception $ex) {
            return json_encode('error : ' . $ex->getMessage());
        ...
        try {
            if ($request->ajax()) {
                ...
                if (!empty($financeMonth_data) and !empty($mainSalaryEmployee_data) and !empty($mainSalaryabsence_data)) {
                    ...
                    $flag = update(new Main_salary_employee_absence(), $dataToUpdate, array('com_code' => $com_code, 'id' => $request->id, 'finance_months_periods_id' => $request->finance_month_period_id, 'main_salary_employee_id' => $request->main_salary_employee_id, 'is_archived' => 0));
    ...
    public function delete(Request $request)
    {
        try {
            if ($request->ajax()) {
                ...
                $mainSalaryabsence_data = get_cols_where_row(new Main_salary_employee_absence(), array('id'), array('com_code' => $com_code, 'id' => $request->id, 'finance_months_periods_id' => $request->finance_month_period_id, 'main_salary_employee_id' => $request->main_salary_employee_id, 'is_archived' => 0));
                if (!empty($financeMonth_data) and !empty($mainSalaryabsence_data) and !empty($mainSalaryEmployee_data)) {
                    DB::beginTransaction();

                    $flag = destroy(new Main_salary_employee_absence(), array('com_code' => $com_code, 'id' => $request->id, 'finance_months_periods_id' => $request->finance_month_period_id, 'is_archived' => 0));
    ...
    // Search absences By EmpCode - absencesType - IsArchived. In the specific FinanceMonth!! -> Ajax
    public function showAjaxSearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Main_salary_employee_absence::select('*')->where($field1, $operator1, $value1)->where($field2, $operator2, $value2)
                ->where(['com_code' => $com_code, 'finance_months_periods_id' => $finance_month_period_id])->orderby('id', 'DESC')->get();

            return view('admin.main_salary_employee_absences.show_ajax_search', ['data' => $data]);
        }
    }

    // Print the absences search result 
    public function printSearch(Request $request)
    {
        $com_code = auth()->user()->id;
        $employee_code = $request->employee_code_search_absence;
        $is_archived = $request->is_archived_search_absence;
        $finance_month_period_id = $request->finance_month_period_id;
        ...
        return view('admin.main_salary_employee_absences.print_search', ['data' => $data, 'financeMonth_data' => $financeMonth_data, 'systemData' => $systemData, 'totals' => $other]);
    }
    ...
    public function ajaxSearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('admin.main_salary_employee_absences.ajax_search', ['data' => $data]);
        }
    }
}
