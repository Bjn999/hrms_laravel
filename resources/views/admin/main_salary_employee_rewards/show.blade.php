    ...
    <div class="modal-dialog modal-2xl">
        <div class="modal-content">
            <div class="modal-header bg-secondary">
                <h4 class="modal-title">اضافة مكافئة للموظفين</h4>
...
<div class="modal fade" id="edit_rewardModal">
    <div class="modal-dialog modal-2xl">
        <div class="modal-content">
            <div class="modal-header bg-secondary">
                <h4 class="modal-title">تعديل مكافئة للموظف</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
            </div>
...
<script>
    $(document).ready(function() {
            ...
            jQuery.ajax({
                ...
                , success: function(data) {
                    ...
                    if (resFlag) {
                        ...
                        jQuery.ajax({
                            url: "{{ route('mainsalaryreward.store') }}"
                            , type: 'post'
                            , 'dataType': 'json'
                            , cache: false
                            , data: {
        ...
        $(document).on("click", ".delete_reward", function(e) {
            ...
            Swal.fire({
                ...
                if (res.isConfirmed) {
                    ...
                    jQuery.ajax({
                        url: "{{ route('mainsalaryreward.delete') }}"
                        , type: 'post'
                        , 'dataType': 'json'
                        , cache: false
                        , data: {
        ...
        $(document).on("click", ".edit_reward", function(e) {
            ...
            $('#loadingModal').modal('show');
            jQuery.ajax({
                url: "{{ route('mainsalaryreward.edit') }}"
                , type: 'post'
                , 'dataType': 'html'
                , cache: false
                , data: {
        ...
        $(document).on("click", "#do_edit_reward", function(e) {
            ...
            $('#loadingModal').modal('show');
            jQuery.ajax({
                url: "{{ route('mainsalaryreward.update') }}"
                , type: 'post'
                , 'dataType': 'json'
                , cache: false
                , data: {
