        ...
        <form action="{{ route('admin_panel_settings.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    @if (!empty($data['image']))
                    @if (file_exists('assets/admin/uploads/'.$data['image']))
                    <img class="com_logo" src="{{ url('assets/admin/uploads').'/'.$data['image'] }}">
                    @else
                    <span class="text-danger mx-3"> خطأ: الصورة غير موجودة </span>
                    @endif
                    @else
                    <img class="com_logo mx-3" src="{{ url('assets/admin/imgs/default_com.png') }}">
                    @endif
