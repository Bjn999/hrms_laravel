                        ...
                        <tbody>
                            @foreach ($data as $info)
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-around align-items-center @if ($info->is_open == 1) bg-success @elseif ($info->is_open == 2) bg-danger @else bg-info @endif"
                                        ...
                                        @if (!empty($info->currentYear))
                                            @if ($info->currentYear['is_open'] == 1)
                                                @if ($info->is_open == 0 and $info->counterOpenMonth == 0 and $info->counterPreviousMonthWaitingOpen == 0)
                                                    <button class="btn btn-primary btn-sm load_the_open_modal"
                                                        data-id="{{ $info->id }}">فتح الان</button>
                                                @endif

                                                @if ($info->is_open == 1)
                                                    <a id="do_close_month"
                                                        href="{{ route('mainsalaryrecord.do_close_month', $info->id) }}"
                                                        class="btn btn-danger btn-sm">إغلاق الشهر</a>
                                                @endif
                                                @if (Session::has('stopped_exist') and Session::get('stopped_exist') == $info->id)
                                                    <a href="{{ route('mainsalaryemployee.show', $info->id) }}"
                                                        class="btn btn-info btn-sm">مراجعة</a>
                                                @endif
                                            @endif
                                        @endif
                                    </td>
...
@section('script')
    <script src="{{ url('assets/admin/js/main_salary_record/index.js') }}"></script>

    <script>
        var ajaxSearchLink = "{{ route('mainsalaryrecord.ajaxSearch') }}";
        var loadOpenMonthLink = "{{ route('mainsalaryrecord.load_open_month') }}";
